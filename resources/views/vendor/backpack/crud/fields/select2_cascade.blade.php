...
<div @include('crud::inc.field_wrapper_attributes') >
    ...
    <select
        name="{{ $field['name'] }}"
        style="width: 100%"
        concatenated="{{ $field['concatenated'] }}"
    ...
    {{-- <div class="form-group">
        ...
        <select name="secondarySelect" class="form-control" style="width:350px">
        </select>
    </div> --}}
</div>
    ...
    @push('crud_fields_scripts')
        ...
        <script>
            ...
            jQuery(document).ready(function($) {
                ...
                $('select[name="{{ $field['concatenated'] }}"]').on('change', function() {
                    // console.log('pippo');
                    var primaryID = $(this).val();
                    if(primaryID) {
                        $.ajax({
                            url: '/api/groupingtypestatuses/'+primaryID,
                            type: "GET",
                            dataType: "json",
                            success:function(data) {
                                $('select[name="{{ $field['name'] }}"]').empty();
                                $.each(data, function(key, value) {
                                    $('select[name="{{ $field['name'] }}"]').append('<option value="'+ value.id +'">'+ value.description +'</option>');
                                });
                            }
                        });
                    }else{
