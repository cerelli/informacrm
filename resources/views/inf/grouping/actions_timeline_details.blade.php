    ...
    @foreach ($actions as $action)
        ...
        <li class="time-label">
            @if (isset($action))
                @php
                // dump($action->pivot);
                    $title = trans('general.grouped_at').' '.ucwords(\Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $action->pivot->created_at)->format('d/m/Y H:i').' ('.$action->pivot->created_at->diffForHumans()).') - '.$action->action_status->description
                @endphp
                <span class="bg" style="background-color: {{ $action->action_status->background_color }} !important; color: {{ $action->action_status->color }} !important;">
                    {{ $title }}
                </span>
            @else
            ...
            <div class="timeline-item">
                <div class="row col-md-12" style="padding-bottom: 10px; padding-top: 5px;">
                    ...
                    <div class="button-tools  col-md-2">
                        @can ('delete action from grouping')
                            <!-- Delete button -->
                            @includeif('inf.buttons.delete', [
                                'custom_button_url' => url(config('backpack.base.route_prefix', 'admin').'/grouping').'/'.$grouping->id.'/action/'.$action->id,
                        ...
                        @can ('update')
                            @includeif('inf.buttons.update', [
                                'custom_button_url' => url(config('backpack.base.route_prefix', 'admin').'/grouping').'/'.$grouping->id.'/action/'.$action->id.'/edit',
