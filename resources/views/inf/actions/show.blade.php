...
@section('header')
	<section class="content-header">
	  <h1>
		<i class="fa {{ config('informa.config.actions_icon') }}"></i>
        <span class="text-capitalize">{{ $crud->entity_name.' ['.$entry->id.']' }}</span>
	  </h1>
	  <div class="pull-right" >
		  @include('inf.acud',['acud' => $entry->acud])
	  </div>
	  {{-- <ol class="breadcrumb"> --}}
	    {{-- <li><a href="{{ url(config('backpack.base.route_prefix'),'dashboard') }}">{{ trans('backpack::crud.admin') }}</a></li> --}}
		{{-- {!! $entry->breadcrumb !!} --}}
	    {{-- <li><a href="{{ url($crud->route) }}?action_type_id={{ $entry->action_types->id }}" class="text-capitalize">{{ $crud->entity_name_plural }}</a></li> --}}
...
<div class="row">
	<div class="col-md-12 ">
		...
		@include('crud::inc.grouped_errors')
		  ...
		  <div class="box box-primary" style="border-top-color: {{ config('informa.config.actions_color') }} !important; border-top-width: 3px;">
		    <div class="box-header with-border">
					<div class="row">
						<div class="col-md-8">
							...
							{{-- ****************ACCOUNT*************** --}}
							<h2 class="name text-light-blue">{!! $entry->getShowAccountLink() !!}</h2>
							@endif
						</div>
						<div class="button-tools  col-md-4">
							<!-- Delete button -->
							@includeif('vendor.backpack.crud.buttons.delete', [
								'custom_button_url' => url(config('backpack.base.route_prefix', 'admin') . '/account/'.$entry->account->id.'/action').'/'.$entry->id,
								'custom_button_attributes' => "  title='Delete action' delete-id='$entry->id' ",
								'custom_button_class' => " pull-right  del-confirmaction"
							])

							<!-- Edit button -->
							@includeif('vendor.backpack.crud.buttons.update', [
								'custom_button_url' => url(config('backpack.base.route_prefix', 'admin') . $entry->editUrl).'/'.$entry->id.'/edit',
				...
				<div class="row">
					<div class="col-md-12">
						{{-- ****************STATUS*************** --}}
						@if ( isset($entry->action_status) )
							<span class="label label-default" style="background-color:{{ $entry->action_status->background_color }}; color:{{ $entry->action_status->color }}">{{ $entry->action_status->description }}</span>
						@endif
					</div>
				</div>
		    </div>
            ...
            <div class="box-footer">
				<div class="tab-container col-md-12">
  			  	<div class="nav-tabs-custom" id="form_tabs">
						...
						{{-- <li role="presentation" class="tab_actions_click">
								<a href="#tab_actions" data-dati="{{ url(config('backpack.base.route_prefix', 'admin') . '/action_tab_actions/'.$entry->id) }}" data-grouping_id="{{ $entry->id }}" data-tab="tab_actions" aria-controls="tab_actions" role="tab" data-toggle="tab">{{ trans('informacrm.actions') }}</a>
						</li> --}}
  			  		</ul>
  			  	</div>
  			  </div>
			  <div class="tab-content col-md-12">
			  	<div role="tabpanel" class="tab-pane active" id="tab_information">
					...
					Note interne
					{{-- @include('inf.actions.thread', ['informations' => $entry]) --}}
					@php
						...
						$internalNote['action_id'] = $entry->id;
						// dd($passFields);
					@endphp
					<div class="row hidden" id="insertInternalNote">
						@include('inf.actions.thread', ['internalNote' => $internalNote])
					</div>
	...
	$(".btn_add_internal_note").click(function(e){
		...
		$.ajax({
			...
			success: function(result) {
				if ( notifing ) {
					new PNotify({
						title: "{{ trans('general.action_thread_confirmation_title') }}",
						text: "{{ trans('general.action_thread__confirmation_message') }}",
						type: "success"
					});
				}
			...
			error: function(result) {
				if ( notifing ) {
					new PNotify({
						title: "{{ trans('general.action_thread_confirmation_not_title') }}",
						text: "{{ trans('general.action_thread_confirmation_not_message') }}",
						type: "warning"
					});
				}
			}
