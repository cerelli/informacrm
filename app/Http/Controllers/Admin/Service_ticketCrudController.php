...
use App\Http\Requests\Service_ticketRequest as StoreRequest;
use App\Http\Requests\Service_ticketRequest as UpdateRequest;

use App\Models\Service_ticket_status;
use App\Models\Service_ticket_type;
use Illuminate\Http\Request;

class Service_ticketCrudController extends CrudController
{
    public function setup()
    {
        ...
        |--------------------------------------------------------------------------
        */
        $this->crud->setModel('App\Models\Service_ticket');
        ...
        */
        ...
        $this->crud->addField([       // Select2Multiple = n-n relationship (with pivot table)
            'label' => trans('informacrm.service_ticket_types').' *',
                ...
                'model' => "App\Models\Service_ticket_type", // foreign key model
            ...
            $this->crud->addField([
                ...
                'type' => 'select',
                'name' => 'service_ticket_status_id', // the db column for the foreign key
                'entity' => 'service_ticket_statuses', // the method that defines the relationship in your Model
                'attribute' => 'description', // foreign key attribute that is shown to user
                'model' => "App\Models\Service_ticket_status", // foreign key model
                'wrapperAttributes' => [
        ...
        $this->crud->addField([
            ...
            'type' => 'select',
            'name' => 'service_ticket_result_id', // the db column for the foreign key
            'entity' => 'service_ticket_results', // the method that defines the relationship in your Model
            'attribute' => 'description', // foreign key attribute that is shown to user
            'model' => "App\Models\Service_ticket_result", // foreign key model
            'wrapperAttributes' => [
        ...
        $this->crud->addFilter([ // select2_multiple filter
          ...
          'label'=> trans('informacrm.service_ticket_status')
                ...
                $statuses = Service_ticket_status::all();
                $statusList = [];
            ...
            if (isset($values)) {
                foreach (json_decode($values) as $key => $value) {
                        if ($key == 0) {
                            $this->crud->addClause('where', 'service_ticket_status_id', $value);
                        } else {
                            $this->crud->addClause('orWhere', 'service_ticket_status_id', $value);
                        }
                }
            }
        });
        ...
        $this->crud->addFilter([ // select2_multiple filter
          ...
          'label'=> trans('informacrm.service_ticket_types')
            ...
            return Service_ticket_type::all()->pluck('description', 'id')->toArray();
        }, function($values) { // if the filter is active
            foreach (json_decode($values) as $key => $value) {
    ...
    public function store(StoreRequest $request)
    {
        ...
        switch ($saveAction) {
            case 'save_and_edit':
                break;
            case 'save_and_new':
                $redirect_location = redirect(config('backpack.base.route_prefix', 'admin').'/service_ticket/create?active_account_id='.$this->crud->entry['account_id']);
                break;
            case 'save_and_back':
            default:
                $redirect_location = redirect('admin/account/'.$this->crud->entry['account_id'].'#service_tickets');
                break;
        }
